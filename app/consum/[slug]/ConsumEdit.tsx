// simplified and cleaned-up final version
"use client";
...
export default function ConsumEdit({ initialData }: { initialData: any }) {
  ...
  //functii pentru afisare si chestii cu numere
  ...
  const stergeLinie = (index: number) => {
    ...
    toast.success(`Produsul "${linieStearsa.bun.nume_bun}" a fost eliminat!`, {
      icon: '🗑️',
    });
  };
  ...
  const editLinie = (index: number) => {
    ...
    toastInfo(`Editare produs "${linie.bun.nume_bun}"`);
  ...
  const salveazaConsum = async () => {
    ...
    try {
      const response = await fetch(`/api/consum/${consum?.id_consum}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const fetchConsum = async () => {
    ...
    try {
      const consumul = await fetch(`http://localhost:3000/api/consum/${consum.id_consum}`).then(res => res.json());
      const cereriData = await fetch('/api/cerere-aprovizionare').then(res => res.json());

      setConsum(consumul)
      setDataConsum(consumul.data)
      setResponsabilSelectat(consumul.sef.id_angajat)
      setGestiuneSelectata(consumul.gestiune.id_gestiune)
      setLiniiConsum(consumul.liniiConsum)
      setCereriAprovizionare(cereriData)
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-gray-800 to-black text-white p-5">
          <h1 className="text-3xl font-serif text-center flex items-center justify-center gap-3">
            ...
            Editare bon de consum nr. {consum?.id_consum}
          </h1>
        </div>
